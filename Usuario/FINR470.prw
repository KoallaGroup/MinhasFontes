...
TRCell():New(oMovBanc,"E5_DTDISPO" ,"SE5",STR0025	,/*Picture*/,12,/*lPixel*/,{|| DtoC(E5_DTDISPO)}) //"DATA"
...
#IFDEF TOP
	...
	WHERE 	%Exp:cSql1%     
			...
			SUBSTRING (SE5.E5_FILIAL,1,2) = SUBSTRING( SA6.A6_FILIAL,1,2) AND // ALTERAÇÃO COFERMETA.
			SE5.%notDel% AND
			SA6.%notDel%
	ORDER BY %exp:cOrder%			
	EndSql               
	...
	While &(cCondWhile)
		...
		If ( lFxMultSld .AND. FXMultSld() )
			nValor   := Round(xMoeda(F470VlMoeda(cAlias),;
										 ...
										 IIf(!lMsmMoeda,RecMoeda(IIf(MV_PAR09==1,dDataBase,E5_DTDISPO),mv_par06),E5_TXMOEDA)),nMoeda)
		Else
			nValor   := Round(xMoeda(F470VlMoeda(cAlias)/*E5_VALOR*/,;
										Iif((cPaisLoc<>"BRA".And.(E5_TIPODOC $ MVRECANT+"|ES") .and. Empty(E5_MOEDA)),1, Val(E5_MOEDA)/*nMoedaBco*/),;
										mv_par06,;
										iif(MV_PAR09==1,dDataBase,E5_DTDISPO),;
										nMoeda+1;
										,Iif(nTxMoedBc > 1 .And. cPaisLoc <> "BRA",nTxMoedBc, if(cPaisLoc=="BRA",IIF(MV_PAR09 == 1, RecMoeda(dDatabase, nMoedaBco ), TxMoeda("SE5", nMoedaBco) ),nTxMoedBc));
