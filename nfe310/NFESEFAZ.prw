...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja) .AND. fCoferOK()//Tratamento para evitar falta de informação de faturas no xml - Marcio Felipe - 30/05/2014 
		...
		If MsSeek(xFilial("SF3")+cClieFor+cLoja+cNota+cSerie)
			While !SF3->(Eof()) .And. cClieFor+cLoja+cNota+cSerie == SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE
				...
				If FunName() == "SPEDNFSE" //.Or. FunName() == "SPEDCTE"
					...
					If SF3->F3_TIPO =="S"
//Cofermeta/Crepaldi - 12/07/2012 = customização da Danfe Cofermeta
/*
						aadd(aISSQN,;
				...
				If SX5->(dbSeek(xFilial("SX5")+"60"+SF3->F3_CODISS))
					...
					If cDescServ$"1"
						...
						If !Empty(cFieldMsg) .and. SC5->(FieldPos(cFieldMsg)) > 0 .and. !Empty(&("SC5->"+cFieldMsg))
							//cServ := &("SC5->"+cFieldMsg) 
							cServ :=  Alltrim(SC6->C6_ZDESCSE) // Customização Cofermeta pegar mensagem do pedido
						Else
							//Cofermeta/Crepaldi - 12/07/2012 = customização da Danfe Cofermeta						
							//cServ := SC5->C5_MENNOTA
							cServ :=  Alltrim(SC6->C6_ZDESCSE)						
						EndIf
						If Empty(cServ)
							cServ := SX5->X5_DESCRI
						EndIf
				...
				#IFDEF TOP
					...
					If SF2->F2_COND == "888"					
						BeginSql Alias cAliasSE1
							...
							SELECT DISTINCT E1_FILIAL,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_VENCORI,E1_VALOR,E1_VLCRUZ,E1_ORIGEM,E1_PIS,E1_COFINS,E1_CSLL,E1_INSS,E1_VLRREAL,E1_IRRF
							FROM %Table:SE1% SE1
							INNER JOIN %Table:P01% P01 ON E1_FILIAL = P01_FILIAL AND E1_PEDIDO = P01_PEDIDO AND RIGHT(E1_PARCELA,2) = P01_PARC //INCLUIDO POR MARCIO FELIPE
							WHERE
							...
							SE1.E1_SALDO > 0 AND P01_FORMA = 'NF' AND // INCLUIDO POR MARCIO FELIPE 
							SE1.%NotDel%
							ORDER BY %Order:SE1%
						EndSql
					Else
...
//Cofermeta/Crepaldi - 12/07/2012 = customização da Danfe Cofermeta
				...
				//Cofermeta/Crepaldi - 12/07/2012 = customização da Danfe Cofermeta
				//Monta mensagem com campo customizado de mensagem
				If SC5->(FieldPos("C5_ZMENNOT")) > 0
					If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
						cMensCli += " "
					EndIf
					
					If !(AllTrim(SC5->C5_ZMENNOT) $ cMensCli)
						cMensCli += AllTrim(SC5->C5_ZMENNOT)
					EndIf
					EndIf
					...
					If SC6->(FieldPos("C6_NUMPCOM")) > 0 .And. SC6->(FieldPos("C6_ITEMPC")) > 0
						...
						Else
							//aadd(aPedCom,{}) COMENTADO POR MARCIO FELIPE - 29/08/2013
						EndIf
					Else
						//aadd(aPedCom,{})  COMENTADO POR MARCIO FELIPE - 29/08/2013
					EndIf
...
//Cofermeta/Crepaldi - 12/07/2012 = Busca no. do pedido de compra do cliente na coluna "C6_PEDCLI" da tabela SC6 - item do pedido de venda.
// Alterado para peger do SC6->C6_DESCNFE e não do C6_PEDCLI. Márcio Felipe 03/04/2013

				SC6->(dbSetOrder(1))
				If (SC6->(MsSeek(xFilial("SC6")+(cAliasSD2)->D2_PEDIDO+(cAliasSD2)->D2_ITEMPV+(cAliasSD2)->D2_COD)))
        			//aadd(aPedCom,{substr(cInfAdic,6,15),substr(cInfAdic,1,5)})  // Ajuste feito por Marcio Felipe para atender ao processo sugerido pela VALE 
					cZItPdCmp := cValToChar(Val(SubStr(MemoLine(SC6->C6_DESCNFE,90,1),4,6))) // Marcio Felipe - 29/08/2013 
					//cZItPdCmp += SubStr(MemoLine(SC6->C6_DESCNFE,90,1),6,4) // Marcio Felipe - 29/08/2013 // comentado 15/10/2013
										
					// Add {Pedido, Item Pedido Compra}
					aadd(aPedCom,{SubStr(MemoLine(SC6->C6_DESCNFE,90,1),10,10),cZItPdCmp}) // Marcio Felipe - 29/08/2013 
				Else
				   aadd(aPedCom,{})
				Endif
...
//Cofermeta/Crepaldi - 12/07/2012 = customização da Danfe Cofermeta
			//Monta mensagem com campo customizado de mensagem
			If SF1->(FieldPos("F1_ZMENNOT")) > 0
				If !(AllTrim(SF1->F1_ZMENNOT) $ cMensCli)
					If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
						cMensCli += " "
					EndIf
					cMensCli += AllTrim(SF1->F1_ZMENNOT)
				EndIf
			EndIf
			dbSelectArea("SD1")
			dbSetOrder(1)	
...
*/              
				...
				SC6->(dbSetOrder(1))
				If (SC6->(MsSeek(xFilial("SC6")+(cAliasSD2)->D2_PEDIDO+(cAliasSD2)->D2_ITEMPV+(cAliasSD2)->D2_COD)))
					cInfAdic := Alltrim(SC6->C6_DESCNFE)
				Else
					cInfAdic := ""		
				Endif
...
//Local lDSaiEnt   := GetNewPar("MV_DSAIENT", .T.) Customizacao cofermeta, abaixo - Marcio Felipe - 17-12-2013
Local lDSaiEnt   := GetNewPar("MV_DSAIENT", .T.) .OR. SC5->C5_TIPLEG $ AllTrim(GetMV("FS_TPLEGDT")) // Impressao da data de saida/entrada de acordo com o tipo legado
Local lNfVincRur := .F.
Local lNfVinc    := .F.
Local lEECFAT		:= SuperGetMv("MV_EECFAT")
...
Do Case
	...
	// Ajuste nota devolução 01/04/2015
	If ((cVeramb >= "3.10" ) .and. ( (aNota[5] $ "D" .And. SF4->F4_AJUSTE <> "S")))
		cTPNota:= "4"
	Endif
...
// FUNCÃO PARA EVITAR QUE A NOTA SEJA TRANSMITIDA ANTES QUE TODAS SUA INFORMAÇÕES SEJAM ATUALIZADAS
Static Function fCoferOK()

Local nCntDelay	:= 1
Local lZRet		:= .T.
    
	// VERIFICA 10 VEZES SE O PEDIDO LEGADO JA FOI PREENCHIDO
	While ALLTRIM(SF2->F2_PEDLEG) == "" .AND. nCntDelay <= 10   
		SLEEP(2000) // Delay de 2000 (2Seg)		
		nCntDelay ++
	EndDo
	
	// SE ESGOTOU O TEMPO LIMITE
	If nCntDelay > 10
		lZRet := .F.
	EndIf
